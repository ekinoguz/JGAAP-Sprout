...
public class Document extends Parameterizable {
	...
	/**
	 * Take the list of canonicizers and apply
	 * them to the document one by one, in the same order they were added.
	 */
	public void processCanonicizers(List<Canonicizer> canonicizerList) throws LanguageParsingException, CanonicizationException {
		if (language.isParseable()){
			text = language.parseLanguage(stringify());
		}
		for (Canonicizer canonicizer : canonicizerList) {
			text = canonicizer.process(text);
		}
	}
